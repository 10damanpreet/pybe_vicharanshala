    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '10px 20px', gap: '10px' }}>
          <button className="sandbox-reset-btn" onClick={() => setShowSandbox(true)}>
            Open Sandbox
          </button>
          {auth.token ? (
            <button className="sandbox-reset-btn" onClick={logout}>
              <LogOut size={16} /> Logout {auth.user?.name}
            </button>
          ) : (
            <button className="primary" onClick={() => setShowAuth(true)} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <User size={16} /> Login to Generate
            </button>
          )}
        </div>
